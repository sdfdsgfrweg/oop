...
private:
	...
	bool hasDerived(BookKey const & _key) const;

	std::map<BookKey,std::unique_ptr<Book>> m_books;

	std::map<std::string, BookKey> m_booksLog;

	std::map<BookKey, int> m_orderLog;
